import { Network, TxParams } from '@thorswap-lib/xchain-client'
import { Client as CosmosClient } from '@thorswap-lib/xchain-cosmos'
import { Chain, assetToString, baseAmount, delay } from '@thorswap-lib/xchain-util'
...
describe('Cosmos Integration Tests', () => {
  ...
  it('should fail xfer xxx from wallet 0 -> 1', async () => {
    try {
      ...
      expect(txResult.raw_log).toEqual(
        'failed to execute message; message index: 0: 0xxx is smaller than 100xxx: insufficient funds',
      )
    } catch (error) {
      throw error
    }
